...
def logbinner(data, n_bins, min_bin=1):
  ...
  for _ in range(n_bins):
    bins.append(np.average(np.abs(data[int(start):math.ceil(stop)])))
    start = stop
    stop = stop * gamma
...
n_max_part = 500
pn = 10
particles = [Dot(rnd()*HEIGHT, rnd()*WIDTH, 0, 0, colorsys.hsv_to_rgb(0,0,1)) for _ in range(pn)]
vmax = 25
...
color_lookup = np.array([[colorsys.hsv_to_rgb(1- x/(WIDTH-space*2), 0.3+0.7*y/spawn_norm, 1) for x in range(int(WIDTH-space*2)+1)] for y in range(spawn_norm+1)])
...
def spawn(plot=True):
  ...
  four_data = logbinner(four_data, n_data_bins, min_bin=1)
  ...
  for x, amp in enumerate(amp_data):
    for y in range(int(amp)):
      ...
      color = color_lookup[y, int(xpos-space)]
      particles.append(Dot(baseline+ypos, xpos, 0, 0, color))
      particles.append(Dot(baseline-ypos, xpos, 0, 0, color))
...
def move_particle(dot):
  ...
  #new_point(start, dest, dot.c)
  line(start, dest, dot.c)
  return True

def new_point(start, dest, color):
  global img
  img[start] = 0
  img[dest] = color

def line(start, dest, color):
  global img
  ...
  for step in range(n):
    img[int(start[0]+step*ydd),int(start[1]+step*xdd)] = color
