...
def spawn(plot=True):
  ...
  four_data = logbinner(four_data, n_data_bins, min_bin=3)/amplitude
  if plot:
    plt.plot(four_data)
    plt.show()
  length = len(particles)
  for y, amp in enumerate(four_data):
    for x in range(int(amp)):
      if length >2000:

        particles.remove(particles[0])
      particles.append(Dot(space+y*y_spread, x*x_spread, 0, 0))
