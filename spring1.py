...
class Dot:
  def __init__(self, y, x, vy, vx, r=1.0):
    self.y = y
    self.x = x
    self.vy = vy
    self.vx = vx
...
spring_stiffness = 0.95 # math.pow(0.02, 1/n_data_bins)
...
def apply_force(dot):
  ...
  dot.vy += yif*fp*f[int(dot.gety()/fhscl),int(dot.getx()/fwscl),0]
  dot.vy += yf*np.sign(yd)
  if dot.vy>vmax: dot.vy = vmax
  if dot.vy<-vmax: dot.vy = -vmax
  dot.vx += xif*fp*f[int(dot.gety()/fhscl),int(dot.getx()/fwscl),1]
  dot.vx += xf*np.sign(xd)
